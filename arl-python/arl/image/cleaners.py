...
from arl.image.cleaners_mod import msmfsclean_kernel_c
...
def overlapIndices(res, psf, peakx, peaky):
    ...
    l, r = (res_lower[0], res_upper[0], res_lower[1], res_upper[1]), \
...
def msmfsclean(dirty, psf, window, gain, thresh, niter, scales, fracthresh, findpeak='CASA', opt=False):
    ...
    # Use optimized algorithm
        ...
        if windowstack is None:
            windowstack_c = numpy.zeros_like(scalestack)
        else:
            windowstack_c = windowstack

        residual = numpy.zeros(m_model.shape)

        msmfsclean_kernel_c(m_model, residual, native_order(scalestack), native_order(smresidual),
                            native_order(ssmmpsf), native_order(hsmmpsf), native_order(ihsmmpsf),
                            native_order(ldirty), native_order(psf), native_order(windowstack_c),
                            gain, absolutethresh, niter, findpeak)

    return m_model, residual
...
def calculate_scale_moment_principal_solution(smresidual, ihsmmpsf):
    ...
    # dir = '/home/yangrunkai/data/ska/arl_code/arl-new/algorithm-reference-library/examples/arl/results/clean/'
    # with open(dir+'smpsoli.dat', 'wb') as fw:
    #     smpsol1d = smpsol.reshape(numpy.prod(smpsol.shape))
    #     for val in smpsol1d:
    #         fw.write(val)
...
def find_optimum_scale_zero_moment(smpsol, windowstack):
    ...
    for scale in range(nscales):
        ...
        # print('zero:', this_max, optimum, sscale, sx, sy)
