...
from arl.imaging.params_mod import get_rowmap_c
...
def get_frequency_map(vis, im: Visibility = None, opt=False):
    ...
    if im is None:
        ...
        if not opt:
            ...
            row2vis = numpy.zeros(vis.frequency.shape, dtype=numpy.int32)
            get_rowmap_c(row2vis, native_order(vis.frequency), native_order(ufrequency))
